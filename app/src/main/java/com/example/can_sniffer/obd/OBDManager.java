package com.example.can_sniffer.obd;
...
public class OBDManager {
    ...
    public OBDManager(OBDSensorListener cListener) {
    ...
    private class OBDCommandListener extends Thread {
        ...
        private boolean InitOBD(InputStream cIn, OutputStream cOut) {
            try {
                ...
                //new ObdRawCommand("ATH0").run(cIn, cOut);//set headers off
                //new ObdRawCommand("ATDP").run(cIn, cOut);
                //new ObdRawCommand("ATM0").run(cIn, cOut);
                //new ObdRawCommand("ATDN").run(cIn, cOut);
                //new ObdRawCommand("ATSP0").run(cIn, cOut);
                new ObdRawCommand("ATSPB").run(cIn, cOut);//Set CAN protocol SAE J1939 CAN (29 bit ID, 250* kbaud)
